    ...
    public static class CityAssaultValidation
    {
        ...
        public static void Validate()
        {
            ...
            if(EditorApplication.isPlaying)
            {
                ...
                Debug.Log("[Sniper Ridge] 왕 처치/4종 무기 조건과 실제 도시 목표/증원 경로 검사 통과");
            }
            else
            {
                ValidateMovement();
                Debug.Log("[Sniper Ridge] 왕 처치/4종 무기 조건·벽 충돌·후진·보도 턱·앉아서 통과 검사 통과. 도시 FPS Play 중 같은 메뉴로 실제 도보/증원 경로를 검사하세요.");
            }
        }
        static void ValidateProgress()
        {
            var progress=new AssaultProgress();
            Check(!progress.Complete && progress.Sector==0,"시작과 동시에 작전 완료");
            Check(!progress.Advance(AssaultLayout.Start),"입구에서 전선이 조기 이동");
            for(int i=0;i<5;i++)
            {
                Check(progress.Advance(AssaultLayout.Objectives[i]+Vector3.forward*7f),"왕에게 전진하는 전선 갱신 실패");
                Check(progress.Sector==i+1 && !progress.Complete,"왕 처치 전에 승리 처리");
            }
            progress.DefeatKing();Check(progress.Complete,"왕 처치 완료 조건 실패");
            var armor=new BossArmor(1200);Check(Mathf.Approximately(armor.Apply(100,false),20),"왕 방어구 피해 흡수 오류");
            for(int i=0;i<14;i++)armor.Apply(100,false);
            Check(armor.Remaining==0 && Mathf.Approximately(armor.Apply(100,true),180),"방어구 파괴 후 헤드샷 피해 오류");
            ...
            inventory.Active.Reserve=0;Check(inventory.SupplyCombat(30,45,5,1)>=45,"적 처치 탄약 보급 실패");
        }
