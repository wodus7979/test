    ...
    public static class TankDriveValidation
    {
        ...
        public static void Validate()
        {
            ...
            try
            {
                ...
                groundData.heightmapResolution=257;groundData.size=new Vector3(500,100,500);
                var hills=new float[257,257];
                for(int z=0;z<257;z++)for(int x=0;x<257;x++)hills[z,x]=TankCanyon.Height(x/256f*500-250,z/256f*500-250)/100f;
                groundData.SetHeights(0,0,hills);
                var terrain=ground.AddComponent<Terrain>();terrain.terrainData=groundData;
                body.constraints=RigidbodyConstraints.None;
                Vector2 start=TankCanyon.Node(2),end=TankCanyon.Node(6);
                body.position=TankCanyon.Ground(terrain,start,.2f);
                body.rotation=Quaternion.LookRotation(new Vector3(end.x-start.x,0,end.y-start.y));
                body.velocity=body.angularVelocity=Vector3.zero;Physics.SyncTransforms();
                Advance(body,physics,0,0,75,terrain);float initialY=body.position.y;
