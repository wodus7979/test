...
## Urban FPS footsteps (2026-09-12)

`fps_footstep.wav`: original synthesized dry pavement step, generated by `Tools/build_fps_footstep.py` with deterministic filtered noise and a short grit layer. No external recording or sampled audio. 48 kHz, stereo PCM16, 0.23 seconds. Used by the free-roaming urban FPS motor at walking/running cadence; not claimed as a real recording.


## Original battle score (2026-09-12)

`bgm_ridge_theme.wav` and `bgm_ridge_battle.wav`: original synthesized score generated by `Tools/build_battle_music.py`. 96 BPM, D minor, 32 bars / 80 seconds, two synchronized stereo 48 kHz PCM16 stems. Harmonic strings/brass-style synthesis, bass, timpani-style drums, snare and cymbal noise; no external recordings, sampled instruments, or copied game soundtrack melodies. This is an original synthesized game cue, not a recorded orchestra. Circular room delays and matched endpoint shoulders preserve the repeating tail. Runtime imports stream Vorbis audio at quality 0.75; the checked-in masters remain PCM.
