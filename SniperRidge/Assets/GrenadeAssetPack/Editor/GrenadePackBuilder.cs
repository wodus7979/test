...
namespace OriginalGrenadeAssets
{
    [InitializeOnLoad]
    public static class GrenadePackBuilder
    {
        const string Root = "Assets/GrenadeAssetPack";
        ...
        public const string Target = "Assets/Resources/Grenades";
        static readonly string[] Names = { "grenade_olive" };
        static bool building;

        static GrenadePackBuilder()
        {
        ...
        [MenuItem("Sniper Ridge/수류탄 프리팹 생성 (Grenade Asset Pack)")]
        public static void Build()
        {
            ...
            try
            {
                ...
                Debug.Log("[Sniper Ridge] 수류탄 프리팹 생성 완료: " + Target);
            }
        ...
        public static Mesh MakeMesh(Level level,string name)
        {
            ...
            foreach(Part p in level.parts)
            {
                ...
                // Source faces are CCW. Reflect Z for Unity clockwise faces, retaining indices.
        ...
        static GameObject MakePrefab(Model model,Material[] materials,string output)
        {
            ...
            try
            {
                ...
                if(model.collision.type=="box")
                {
                    ...
                    collider.size=new Vector3(model.collision.size[0],model.collision.size[1],model.collision.size[2]);collider.enabled=false;
                }
    ...
    public sealed class GrenadeBuildCheck : IPreprocessBuildWithReport
    {
        public int callbackOrder => -100;
        public void OnPreprocessBuild(BuildReport report) => GrenadePackBuilder.BuildIfMissing();
    }
}
