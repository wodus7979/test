...
function cityLab(plan, st) {
  ...
  // Y자 관 — 두 그릇 바닥 구멍에서 45도 계단으로 내려와 가운데(J)에서 만나고,
  // 줄기를 타고 내려가 배수구로 빠진다. 앞뒤 벽은 유리라 흐르는 게 보인다.
  // 계단은 한 칸 옆으로 갈 때마다 한 칸 떨어지므로, 옆으로만 퍼질 때 줄어드는
  // 단계(물 7칸·용암 3칸)에 걸리지 않고 끝까지 흘러간다.
  const FX = -LAB_GAP, CX = LAB_GAP;
  const cells = [];                                   // 관 속 빈 칸 (상대 좌표)
  for (let k = 0; k <= LAB_GAP - 1; k++) {
    const y = Y0 - 1 - k;
    cells.push([FX + k, y], [FX + k + 1, y]);          // 용광로 팔 (왼쪽에서 가운데로)
    cells.push([CX - k, y], [CX - k - 1, y]);          // 컵 팔 (오른쪽에서 가운데로)
  }
  const JY = Y0 - LAB_GAP;                            // 합류점 높이 (양 팔 마지막 칸이 x=0)
  for (let y = JY - 1; y >= JY - LAB_STEM; y--) cells.push([0, y]);   // 줄기
  const drainY = JY - LAB_STEM - 1;
  cells.push([0, drainY]);                            // 배수구 — 여기 닿은 유체는 빠져나간다
  // 껍데기: 빈 칸 둘레는 돌, 앞뒤(z±1)는 유리
  const open = {};
  cells.forEach(function (c) { open[c[0] + ',' + c[1]] = true; });
  cells.forEach(function (c) {
    for (let dy = -1; dy <= 1; dy++) for (let dx = -1; dx <= 1; dx++) for (let dz = -1; dz <= 1; dz++) {
      const x = c[0] + dx, y = c[1] + dy;
      if (dz === 0 && open[x + ',' + y]) continue;
      set(x, y, LZ + dz, dz === 0 ? stone : glassB);
    }
  });
  cells.forEach(function (c) { set(c[0], c[1], LZ, 0); });
  set(0, drainY - 1, LZ, iron);                        // 배수구 바닥은 쇠
  ...
  plan.lab = {
    ...
    cells: cells.map(function (c) { return [ox + c[0], gy + c[1], plan.z + LZ]; }),   // 관 속 빈 칸 (세계 좌표)
    drain: [ox, gy + drainY, plan.z + LZ],
    junction: [ox, gy + JY, plan.z + LZ]
  };
}
...
Game.prototype.labClear = function (lab) {
  ...
  let n = 0;
  for (let i = 0; i < lab.cells.length; i++) {
    const c = lab.cells[i];
    const id = w.getBlock(c[0], c[1], c[2]);
    if (id !== 0 && clear.indexOf(id) >= 0) { w.setBlock(c[0], c[1], c[2], 0, 0, true); n++; }
  }
  // 그릇 구멍 바로 아래 한 칸에 남은 것도
  for (let i = 0; i < lab.plugs.length; i++) {
    const p = lab.plugs[i], id = w.getBlock(p[0], p[1] - 1, p[2]);
    if (id !== 0 && clear.indexOf(id) >= 0) w.setBlock(p[0], p[1] - 1, p[2], 0, 0, true);
  }
  return n;
};
...
Game.prototype.updateLab = function (dt) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    // 배수구 — 닿은 유체는 빠져나간다 (관 밖으로 넘치지 않게)
    const d = lab.drain, did = w.getBlock(d[0], d[1], d[2]);
    if (did === B.water || did === B.lava) w.setBlock(d[0], d[1], d[2], 0);
    // 관 속에서 용암 곁에 물이 있는 자리마다 수증기 (앞 유리 밖으로 새어 오른다)
    let n = 0;
    for (let i = 0; i < lab.cells.length && n < 8; i++) {
      const c = lab.cells[i], x = c[0], y = c[1], z = c[2];
      if (w.getBlock(x, y, z) !== B.lava) continue;
      if (w.getBlock(x + 1, y, z) === B.water || w.getBlock(x - 1, y, z) === B.water ||
          w.getBlock(x, y + 1, z) === B.water || w.getBlock(x, y - 1, z) === B.water) {
        this.fx.steam(x + 0.5, y + 0.6, z - 1.2, 1, false);
        n++;
      }
    }
  }
