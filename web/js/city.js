...
function buildCityPlan(world, ap, index, code) {
  ...
  if (def.style === 'skyline') {
    ...
    // 격납고는 '가장 높은 건물' 위에 얹는다 — 롯데타워도 후보에 넣는다
    if (!plan.tallest || lotteH > plan.tallest.h) {
      plan.tallest = { x: best.x + plaza.x, z: best.z + plaza.z,
                       gy: gy, h: lotteH, hw: 8, hd: 8 };
    }
    for (let k = 0; k < 8; k++) {
  ...
  // ── 강철 슈트 격납고 ──
  // 도시에서 가장 높은 건물에 둔다. 들어가는 길은 두 곳:
  //  1) 1층 출동실 — 큰길에서 걸어 들어가면 바로 슈트를 입는다 (가장 쉬운 길)
  //  2) 옥상 착륙장 — 넓게 트인 발판이라 하늘에서도 쉽게 내려앉는다
  // 예전에는 옥상에 유리방 하나뿐이라, 90칸 위 7×7 상자에 정확히 날아들어야
  // 했고 3인칭 카메라가 유리 안에 갇혀 화면이 온통 유리로 덮였다.
  (function () {
    let t = plan.tallest;
    if (!t && plan.landmark) {
      // 높은 건물이 없는 도시는 랜드마크 꼭대기에 얹는다
      t = { x: Math.round(plan.landmark.x), z: Math.round(plan.landmark.z),
            gy: gy, h: Math.max(4, Math.round(plan.landmark.y - gy) - 2), hw: 3, hd: 3 };
    }
    if (!t) return;

    const floorB = bid('gray_concrete', 'stone');
    const trim = bid('gold_block', 'yellow_concrete');
    const lamp = bid('sea_lantern', 'glowstone');
    const wall = bid('black_concrete', 'stone');
    const rail = bid('light_blue_stained_glass_pane', 'glass_pane');

    // ── 옥상 착륙장 ──
    const roof = t.gy + t.h, base = roof + 1, D = 6;      // 13×13
    for (let dz = -D; dz <= D; dz++) {
      for (let dx = -D; dx <= D; dx++) {
        const r = Math.max(Math.abs(dx), Math.abs(dz));
        plan.set(t.x + dx, base, t.z + dz, (r === D) ? trim : floorB, 0, true);
        // 밑을 계단처럼 좁혀 탑 위에 얹힌 것처럼 보이게 한다
        if (r <= D - 2) plan.set(t.x + dx, base - 1, t.z + dz, floorB, 0, true);
        if (r <= D - 4) plan.set(t.x + dx, base - 2, t.z + dz, floorB, 0, true);
      }
    }
    // 발판 — 여기 서면 슈트를 입는다. 첨탑이 한가운데를 지나므로 조금 앞에 둔다.
    const padZ = t.z + 3;
    for (let dz = -1; dz <= 1; dz++) {
      for (let dx = -1; dx <= 1; dx++) plan.set(t.x + dx, base, padZ + dz, lamp, 0, true);
    }
    for (let dz = -2; dz <= 2; dz++) {
      for (let dx = -2; dx <= 2; dx++) {
        if (Math.max(Math.abs(dx), Math.abs(dz)) === 2) plan.set(t.x + dx, base, padZ + dz, trim, 0, true);
      }
    }
    // 발판 위는 비워 둔다 (첨탑 곁가지에 머리가 걸리지 않게)
    for (let dz = -1; dz <= 1; dz++) {
      for (let dx = -1; dx <= 1; dx++) {
        for (let y = 1; y <= 3; y++) plan.set(t.x + dx, base + y, padZ + dz, 0, 0, true);
      }
    }
    // 가장자리 난간 — 가운데는 터놓아 어느 쪽으로도 날아 들어올 수 있다
    for (let d = -D; d <= D; d++) {
      if (Math.abs(d) <= 2) continue;                       // 드나드는 문
      plan.set(t.x + d, base + 1, t.z - D, rail, 0, true);
      plan.set(t.x + d, base + 1, t.z + D, rail, 0, true);
      plan.set(t.x - D, base + 1, t.z + d, rail, 0, true);
      plan.set(t.x + D, base + 1, t.z + d, rail, 0, true);
    }
    // 네 귀퉁이 등대 — 멀리서도 보이라고 높이 세운다
    for (let sx = -1; sx <= 1; sx += 2) {
      for (let sz = -1; sz <= 1; sz += 2) {
        const px = t.x + sx * D, pz = t.z + sz * D;
        plan.set(px, base + 1, pz, trim, 0, true, 4);
        plan.set(px, base + 5, pz, lamp, 0, true);
      }
    }

    // ── 1층 출동실 ── (건물 앞면을 파고 들어간다)
    const gd = t.hd, gw = Math.min(2, Math.max(1, t.hw - 1));
    const zFace = t.z + gd;                                  // 앞면 (+Z 쪽)
    const deep = 5;
    for (let k = 0; k < deep; k++) {
      const z = zFace - k;
      for (let dx = -gw; dx <= gw; dx++) {
        plan.set(t.x + dx, t.gy, z, floorB, 0, true);        // 바닥
        for (let y = 1; y <= 3; y++) plan.set(t.x + dx, t.gy + y, z, 0, 0, true);   // 속을 비운다
        plan.set(t.x + dx, t.gy + 4, z, (k === 0) ? trim : wall, 0, true);          // 천장
      }
      // 옆벽
      for (let y = 1; y <= 3; y++) {
        plan.set(t.x - gw - 1, t.gy + y, z, (k === 0) ? trim : wall, 0, true);
        plan.set(t.x + gw + 1, t.gy + y, z, (k === 0) ? trim : wall, 0, true);
      }
    }
    // 안쪽 벽과 발판
    for (let dx = -gw; dx <= gw; dx++) {
      for (let y = 1; y <= 3; y++) plan.set(t.x + dx, t.gy + y, zFace - deep, wall, 0, true);
      plan.set(t.x + dx, t.gy, zFace - deep + 1, lamp, 0, true);
    }
    // 문 옆 등 — 밤에도 눈에 띈다
    plan.set(t.x - gw - 1, t.gy + 3, zFace, lamp, 0, true);
    plan.set(t.x + gw + 1, t.gy + 3, zFace, lamp, 0, true);

    plan.hangar = { x: t.x + 0.5, y: base + 1, z: padZ + 0.5 };            // 옥상 발판
    plan.gate = { x: t.x + 0.5, y: t.gy + 1, z: zFace - deep + 1 + 0.5 }; // 1층 발판
  })();
