...
Player.prototype.update = function (dt, input) {
  ...
  if (this.flying) {
    ...
    const control = (this.web || this.suit || this.zipUntil > 0) ? 0
  ...
  if (!this.flying) {
    if (this.suit && this.suitFly) this.suitFly(dt, input);
    else if (this.web && this.webSwing) this.webSwing(dt, input);
  ...
  if (this.onGround && !wasGround) {
    if (this.fallStart !== null) {
      ...
      if (dist > 3 && !this.creative && !this.suit) this.hurt(Math.floor(dist - 3), '낙하');
    }
    this.fallStart = null;
  }
