...
function imLean(p) {
  if (!p.suit) return 0;
  return IM_LEAN_MAX * (p.suit.prone || 0);
}
...
const IM_AIM_HOLD = 0.45;             // 겨눈 팔이 내려오기까지 (초)

// 겨누는 정도 0~1 — 광선을 쏜 쪽 팔만 든다
function imAimOf(p, side) {
  if (side < 0 || !p.suit) return 0;
  return Math.max(0, Math.min(1, p.suit.aim || 0));
}

// 팔이 향하는 X축 각. 겨눌 때는 보는 쪽으로 쭉 뻗는다.
// 팔이 아래로 늘어진 (0,-1,0) 을 A 만큼 돌리면 (0, -cosA, -sinA) 을 향한다.
// 시선은 lookDir 기준으로 (0, sin pitch, cos pitch) 이므로 A = -90도 - pitch.
function imArmAngle(p, lean, side) {
  const base = lean + imArmSwing(lean);
  const a = imAimOf(p, side);
  if (a <= 0) return base;
  const aim = -Math.PI / 2 - (p.pitch || 0);
  return base + (aim - base) * a;
}

// 팔 기준 좌표(어깨가 원점) → 세계 방향. 벌림(Z) → 젖힘·눕기·겨눔(X) → 몸 방향(Y)
function imArmTurn(p, lean, side, lx, ly, lz, out) {
  const a = IM_ARM_SPREAD * side * (1 - 0.75 * imAimOf(p, side));
  const ca = Math.cos(a), sa = Math.sin(a);
  const x1 = lx * ca - ly * sa, y1 = lx * sa + ly * ca;
  return imTurn(p, imArmAngle(p, lean, side), x1, y1, lz, out);
}
...
Game.prototype.suitBeam = function () {
  ...
  p.suit.aim = 1;                 // 오른팔을 들어 겨눈다 (그림보다 먼저 세워야
                                  //  광선이 든 손끝에서 나간다)
  ...
  // raycast 는 빗나가도 {hit:false} 를 돌려준다. hit 를 보지 않으면 좌표가
  // NaN 이 되어 광선이 아예 그려지지 않는다.
  const bh = this.world.raycast(eye[0], eye[1], eye[2], dir[0], dir[1], dir[2], best);
  if (bh && bh.hit && !hitMob) hitAt = [bh.x + 0.5, bh.y + 0.5, bh.z + 0.5];
...
const IM_CAM_AIM = 8;      // 겨누는 쪽을 이만큼 앞에서 바라본다
Game.prototype.suitCamera = function (dt) {
  const p = this.player;
  // 시선은 lookDir 과 같은 규약이어야 한다 — 위를 보면 (+pitch) 카메라가
  // 아래로 내려가 올려다본다. 부호가 뒤집혀 있으면 겨눈 쪽과 화면이 어긋난다.
  ...
  // 슈트가 아니라 '겨누는 쪽'을 바라본다. 슈트만 바라보면 손에서 나간 광선이
  // 화면 밖으로 벗어나 어디를 쏘는지 보이지 않는다. 슈트는 화면 아래쪽에 남는다.
  const aim = [target[0] + fx * IM_CAM_AIM,
               target[1] + fy * IM_CAM_AIM,
               target[2] + fz * IM_CAM_AIM];
