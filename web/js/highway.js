...
Highway.prototype.build = function () {
  ...
  // ── 호수 자리 ──
  // 예전에는 "첫 두 도시 사이 한가운데"에 앉혔는데, 서울과 인천 송도처럼
  // 실제로도 붙어 있는 두 도시를 고르면 호수가 시가지 한복판을 덮어
  // 빌딩이 물에 잠긴 채로 서 있었다. 그래서 제일 멀리 떨어진 두 도시를
  // 골라 그 사이에 앉히고, 자리 고르기 단계에서 도시·공항을 피한다.
  let a0 = order[0], b0 = order[1], bd = -1;
  for (let i = 0; i < order.length; i++) {
    for (let j = i + 1; j < order.length; j++) {
      const d = Math.hypot(order[i].x - order[j].x, order[i].z - order[j].z);
      if (d > bd) { bd = d; a0 = order[i]; b0 = order[j]; }
    }
  }
  ...
  // 호수 위를 다리로 건너는 길은 "호수를 앉힌 그 두 도시" 사이 길뿐이다.
  // 아무 길에나 호수를 끼우면 엉뚱한 노선이 수천 칸을 돌아간다.
  let lakeLink = -1;
  for (let k = 0; k < links.length; k++) {
    const A = order[links[k][0]], B = order[links[k][1]];
    if ((A === a0 && B === b0) || (A === b0 && B === a0)) { lakeLink = k; break; }
  }
...
// 호수가 물러나 있어야 할 거리 — 물가 띠 끝에서 도시 가장자리까지
const LAKE_CITY_KEEP = LAKE_R + LAKE_MARGIN + CITY_R + CITY_MARGIN + 20;

Highway.prototype.pickLake = function (mx, mz, ux, uz, ul) {
  const nx = uz, nz = -ux;                    // 두 도시를 잇는 선의 직각 방향
  const span = Math.min(ul * 0.22, 700);
  const cities = this.world.cities ? this.world.cities() : [];
  const aps = this.world.airports ? this.world.airports() : [];
  // 도시나 공항 부지를 건드리는 자리인가
  const blocked = function (x, z) {
    for (let i = 0; i < cities.length; i++) {
      if (Math.hypot(cities[i].x - x, cities[i].z - z) < LAKE_CITY_KEEP) return true;
    }
    for (let i = 0; i < aps.length; i++) {
      if (Math.abs(aps[i].x - x) < AP_X + LAKE_R + LAKE_MARGIN &&
          Math.abs(aps[i].z - z) < AP_Z + LAKE_R + LAKE_MARGIN) return true;
    }
    return false;
  };
  let best = null;
  ...
  // 어디에도 앉힐 자리가 없으면 호수를 만들지 않는다 (도시를 덮느니 없는 편이 낫다)
  if (!best) return null;
  ...
  if (lk && Math.abs(bx + 8 - lk.x) <= LAKE_R + LAKE_MARGIN + 16 &&
    ...
    for (let lz = 0; lz < CHUNK_Z; lz++) {
      for (let lx = 0; lx < CHUNK_X; lx++) {
        ...
        // 도시 부지 안은 절대 손대지 않는다 — 물이 빌딩을 덮는다
        if (hw.nearCity(wx, wz, CITY_R + CITY_MARGIN)) continue;
