...
SRC=ROOT/'Unity/Assets/NatureFPSPack/Source'
FONT=os.environ.get('NATURE_KOREAN_FONT','/System/Library/Fonts/AppleSDGothicNeo.ttc')
...
def preview(items,path,size=(1600,1100),eye=(155,145,-180),target=(0,0,0),title='산 · 나무 · 들판 / Unity 자연 환경 에셋',clip_roofs=False,lod=0):
 ...
 for item in items:
  ...
  for p in models[item['asset']]['lods'][lod]['parts']:
   ...
   vs=((pos*np.array(item.get('scale',[1,1,1])))@r.T+np.array(item.get('position',[0,0,0])))@rotation.T
  ...
  for i in range(0,len(vs),3):
   ...
   if mat.get('atlas'):
    ...
    if mat.get('albedo'):
     ...
     mip_lod=max(0,math.log2(max(1,footprint)))
     color=sample(cache[mat['albedo']],u,v,mip_lod)**2.2*np.array(mat['color'])
     if abs(det)>1e-10:
      tangent=norm((dpos[0]*duv[1,1]-dpos[1]*duv[0,1])/det)
      bitangent=norm((dpos[1]*duv[0,0]-dpos[0]*duv[1,0])/det)
      tn=sample(cache[mat['normalGltf']],u,v,mip_lod)*2-1
 ...
 d.text((44,70),('LOD'+str(lod)+' 실제 메시 / 자체 렌더 · Unity 화면 아님'),font=ImageFont.truetype(FONT,19),fill=(168,185,192))
 im.save(path,quality=95);print(path.name,flush=True)


if __name__=='__main__':
 preview([{'asset':'broadleaf_1','position':[-8,0,0]},{'asset':'broadleaf_2','position':[0,0,0]},{'asset':'pine_1','position':[9,0,0]},{'asset':'pine_2','position':[17,0,0]}],ROOT/'tree_lineup.jpg',(1900,1150),(20,9,-55),(4,7,0),title='활엽수 2종 · 침엽수 2종',lod=0)
 preview(layout['instances'],ROOT/'nature_overview.jpg',(1800,1300),(150,128,-180),(0,15,0),lod=1)
 preview([{'asset':'terrain_rocky','position':[0,0,0]}],ROOT/'mountain_detail.jpg',(1600,1100),(135,95,-140),(0,20,0),title='암산 지형 · 바위 / 풀밭 텍스처',lod=0)
 preview([{'asset':'bush','position':[0,0,0]},{'asset':'grass_short','position':[3,0,0]},{'asset':'grass_tall','position':[5,0,0]},{'asset':'boulder_1','position':[9,0,0]},{'asset':'boulder_2','position':[13,0,0]}],ROOT/'ground_cover.jpg',(1800,950),(16,7,-25),(5,1,0),title='관목 · 풀 2종 · 바위 2종',lod=0)
